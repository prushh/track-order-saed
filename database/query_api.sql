...
SELECT orders.id, orders.total_cost
FROM orders
...
/* GET all orders with tracking_id */
...
/* GET all orders without tracking_id by user_id */
...
/* UP status for trackings by tracking_id and status_id */
UPDATE trackings
SET status_id = <status_id>
WHERE trackings.id = <tracking_id>

/* INS add tracking by order_id and tracking_id */
INSERT INTO trackings VALUES (default, <courier>, 1)
UPDATE orders
SET orders.tracking_id = <last>
WHERE orders.id = <order_id>
/* GET last id in trackings */
SELECT MAX(trackings.id) AS last
FROM trackings;

/* DEL tracking_id by order_id and tracking_id*/
UPDATE orders
